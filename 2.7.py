...
def improved_model_training(X_train, X_test, y_train, y_test):
    ...
    lr_mse, lr_r2 = train_and_evaluate_model(lr_model, X_train, X_test, y_train, y_test)
    ...
    ridge_mse, ridge_r2 = train_and_evaluate_model(best_ridge_model, X_train, X_test, y_train, y_test)
    ...
    grid_search_lasso = GridSearchCV(estimator=lasso_model, param_grid=param_grid_lasso, cv=5, scoring='neg_mean_squared_error', n_jobs=-1)
    grid_search_lasso.fit(X_train, y_train)
    best_lasso_model = grid_search_lasso.best_estimator_
    lasso_mse, lasso_r2 = train_and_evaluate_model(best_lasso_model, X_train, X_test, y_train, y_test)
...
def main():
    ...
    # 改进后的模型训练和评估
    print("开始改进后的模型训练和评估...")
    results = improved_model_training(X_train_processed, X_test_processed, y_train, y_test)
